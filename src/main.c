...
struct Mat4 lookat(struct Vec3f eye, struct Vec3f center, struct Vec3f up){
...
int main(int argc, char* argv[]){
    ...
    struct Mat4  ModelView = lookat(eye, center, up);
    struct Vec3f light_dir = {0, 0, -1};
    struct Mat4 Projection = identityMat4();
    //viewport is used for moving cordinates to screen cordiantes and scaling them
    //appropriatley
    struct Mat4 ViewPort = viewport(WIDTH/8, HEIGHT/8, WIDTH*3/4, HEIGHT*3/4);
    struct Vec3f z = vector_sub3f(eye, center);
    Projection.data[3][2] = -1.0f/magnitude3f(z);
    ...
    for(int i = 0; i < model.nfaces; i++){
        ...
        for(int j = 0; j < 3;j++){
            ...
            //model and view matrix already multipled
